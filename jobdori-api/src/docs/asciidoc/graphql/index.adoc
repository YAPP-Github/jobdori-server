...
아래 표에는 각 GraphQL operation에서 도메인 로직에 따라 발생할 수 있는 커스텀 에러 코드만 명시합니다.

=== Notion 도메인 에러 코드

Notion 연결, 페이지 검색, 경험 가져오기 과정에서 발생할 수 있는 Notion 도메인 에러 코드입니다.

include::./notion/notion-error.adoc[]

=== Operation별 에러 코드

include::./operation-error.adoc[]
