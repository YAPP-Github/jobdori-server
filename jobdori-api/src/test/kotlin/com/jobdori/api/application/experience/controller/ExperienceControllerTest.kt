...
internal class ExperienceControllerTest(
    ...
    "경험을 생성한다" {
        mockMvc.post("/api/v1/workspaces/{workspaceId}/experiences", "workspace-id") {
    ...
    "경험을 수정한다" {
        mockMvc.patch("/api/v1/workspaces/{workspaceId}/experiences/{experienceId}", "workspace-id", 5L) {
            header(HttpHeaders.AUTHORIZATION, "Bearer access-token")
            contentType = MediaType.APPLICATION_JSON
            content = JsonUtils.toJson(
    ...
    "경험을 삭제한다" {
        mockMvc.delete("/api/v1/workspaces/{workspaceId}/experiences/{experienceId}", "workspace-id", 5L) {
            header(HttpHeaders.AUTHORIZATION, "Bearer access-token")
        }.andExpect {
            status { isOk() }
            jsonPath("$.ok") { value(true) }
