...
internal class ExperienceProjectControllerTest(
    ...
    "경험 프로젝트를 생성한다" {
        mockMvc.post("/api/v1/workspaces/{workspaceId}/experience-projects", "workspace-id") {
    ...
    "경험 프로젝트를 수정한다" {
        mockMvc.patch("/api/v1/workspaces/{workspaceId}/experience-projects/{projectId}", "workspace-id", 3L) {
            header(HttpHeaders.AUTHORIZATION, "Bearer access-token")
            contentType = MediaType.APPLICATION_JSON
            content = JsonUtils.toJson(
    ...
    "경험 프로젝트를 삭제한다" {
        mockMvc.delete("/api/v1/workspaces/{workspaceId}/experience-projects/{projectId}", "workspace-id", 3L) {
            header(HttpHeaders.AUTHORIZATION, "Bearer access-token")
        }.andExpect {
            status { isOk() }
            jsonPath("$.ok") { value(true) }
