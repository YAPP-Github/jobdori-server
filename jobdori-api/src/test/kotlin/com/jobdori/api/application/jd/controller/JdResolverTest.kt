...
internal class JdResolverTest(
    ...
    "붙여넣기(body)로 등록하면 sourceUrl 없이 추출된 JD의 전체 필드를 반환한다" {
        val body = "여러 공고가 섞이지 않은 단일 본문 ".repeat(50)
        every { registerJdService.registerByText(10L, body) } returns
            JdRegisterResult.Registered(
                Jd(
                    id = 1L,
                    publicId = "jd-pub-9",
        ...
        authenticatedTester(graphQlTester)
            .document(
                """
                mutation {
                  registerJd(workspaceId: "ws-1", request: { body: "$body" }) {
                    jd {
                      jdId
                      sourceUrl
                      companyName
            ...
            .execute()
            .path("registerJd.jd.jdId").entity<String>().isEqualTo("jd-pub-9")
            .path("registerJd.jd.sourceUrl").valueIsNull()
            .path("registerJd.jd.companyName").entity<String>().isEqualTo("잡도리")
            .path("registerJd.jd.companyIntro").entity<String>().isEqualTo("채용 도우미 팀")
            .path("registerJd.jd.responsibilities").entityList(String::class.java).containsExactly("API 설계")
            .path("registerJd.jd.requiredExperiences").entityList(String::class.java).containsExactly("Kotlin 3년 이상")
            .path("registerJd.jd.preferredExperiences").entityList(String::class.java).containsExactly("Spring Boot 경험")
            .path("registerJd.jd.hiringProcess").entityList(String::class.java).containsExactly("서류", "면접")
            .path("registerJd.jd.status").entity<String>().isEqualTo("IN_PROGRESS")

        verify(exactly = 1) { registerJdService.registerByText(10L, body) }
    }
