...
import com.jobdori.core.application.auth.token.AuthTokenPayload
...
class AuthUserReaderTest : StringSpec({
    ...
    val authUserReadService = AuthUserReadService(authTokenProvider, userReader)
    ...
    "Access 토큰의 사용자 존재를 검증하고 userId를 반환한다" {
        ...
        // when & then
        authUserReadService.getUserId("access-token") shouldBe 10L
    ...
    "토큰의 사용자가 존재하지 않으면 인증 실패로 처리한다" {
        ...
        shouldThrow<InvalidAuthTokenException> {
            authUserReadService.getUserId("access-token")
        }
    }
